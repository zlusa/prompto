...
from glue.promptopt.constants import PromptOptimizationLiterals
from glue.promptopt.techniques.common_logic import DatasetSpecificProcessing
from glue.promptopt.utils import get_promptopt_class
...
class GluePromptOpt:
    ...
    def __init__(self, llm_config_path: str,
        ...
        set_logging_config(join(base_path,
                                self.setup_config.dir_info.log_dir_name),
        ...
        self.logger.info(f"LLM configurations parameters: {llm_config}")
    ...
    def get_best_prompt(self) -> (str, Any):
        ...
        self.BEST_PROMPT, self.EXPERT_PROFILE = self.prompt_opt.get_best_prompt(self.prompt_opt_param)

        self.logger.info(f"Time taken to find best prompt: {(time.time() - start_time)} sec")
        self.logger.info(f"Summary on LLM tokens used in training phase:\n "
                         f"{LLMMgr.get_tokens_used(self.prompt_opt.llm_pool[self.prompt_opt.setup_config.assistant_llm.prompt_opt])}")
    ...
    def evaluate(self, test_dataset_jsonl: str) -> float:
        ...
        self.logger.info(f"Summary on LLM tokens used for evaluation:\n "
                         f"{LLMMgr.get_tokens_used(self.prompt_opt.llm_pool[self.prompt_opt.setup_config.assistant_llm.prompt_opt])}")
